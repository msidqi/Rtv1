...
int				ft_cone_inter(t_ray *ray, t_cone *co)
{
	...
	k = ft_vec4_sub(ray->origin, co->center);
	abc[0] = pow(ft_vec4_dot_product(ray->dir, co->axis), 2)
		- pow(cos(co->half_angle), 2);
	abc[1] = 2 * (ft_vec4_dot_product(ray->dir, co->axis)
		* ft_vec4_dot_product(k, co->axis) - ft_vec4_dot_product(ray->dir, k)
			* pow(cos(co->half_angle), 2));
	abc[2] = pow(ft_vec4_dot_product(k, co->axis), 2)
		- ft_vec4_dot_product(k, k) * pow(cos(co->half_angle), 2);
