...
static	void	ft_diffuse(t_shader_x *shx, double dot_prod, t_vec4 *ds, t_light *li, double area_i)
{
...
double	ft_light_intensity(t_list *scene, t_light *li, t_vec4 inter_point, t_vec4 ray_to_light)
{
	if (li->type == POINT_LIGHT)
		return (1);
	else if (li->type == DIRECTIONAL_LIGHT)
	...
	else if (li->type == AREA_LIGHT)
	{
		...
		// t_vec4 dir = ft_vec4_normalize(ft_vec4_cross_product(u, v));
		double	v_nodes = 20;
		double	u_nodes = 2;
		...
		double distance_to_light, t_light *li)
{
	...
	while (scene != NULL)
	{
		r_light->t = FAR;
		i = -1;
		while (++i < (unsigned int)STATIC_ARRAY_SIZE(g_t_obj_functions)) // why shadow ray does't intersect with objects
		{
			if (g_t_obj_functions[i].type == scene->content_size &&
					g_t_obj_functions[i].call(r_light, scene->content) &&
					distance_to_light > ft_vec4_mag(ft_vec4_scalar(r_light->dir, r_light->t))){
				// printf("no intersect\n");
				return (0); // ray towards light intersected with an obj
				}
		}
		scene = scene->next;
	}
	return (ft_light_intensity(list, li, r_light->origin, r_light->dir)); // ray reached the light
}
