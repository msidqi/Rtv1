...
/*   Updated: 2019/09/20 15:54:29 by kdaou            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_stock_box_config_next(char *line, t_box *p, int *i)
{
	...
	if (ft_expect_matrix(line, "\ttransfo ", &(p->bound_max)))
		j++;
	if (ft_expect_matrix(line, "\ttransfo ", &(p->bound_min)))
		j++;
	if (j == 2)
		*i += 32;
	return (j == 2 || tmp ? 1 : 0);
...
{
	if (!(*i & 1) && ft_expect_vector(s, "\tbound_max", &(p->bound_max)))
		*i += 1;
		...
		*i += 8;
	else if (!(*i & 16) && ft_expect_vector(s, "\tbound_min", &(p->bound_min)))
		*i += 16;
...
void		ft_get_box_config(int fd, t_data *data)
{
	...
	while (get_next_line(fd, &line) > 0)
	{
		if (ft_bracket_control(line, '{'))
		{
			...
			{
				ft_memdel((void **)&box);
				ft_error_management(data, 11, (void **)&line, fd);
			}
			else
			{
				ft_strdel(&line);
