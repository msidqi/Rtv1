...
/*   Updated: 2019/07/16 17:11:52 by kdaou            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void ft_cone_val_correction(t_cone **cone, t_data *data, int fd)
{
	...
	(*cone)->axis = ft_vec4_normalize((*cone)->axis);					// correcting normal
	dot_prod = ft_vec4_dot_product((*cone)->axis, up);
	if (dot_prod <= 1 + NEAR && dot_prod >= 1 - NEAR)
		up = ft_create_vec4(1, 0, 0, 0);
	(*cone)->left = ft_vec4_normalize(
							ft_vec4_cross_product((*cone)->axis, up));	// calculate left && forward vectors
	...
	{
		if (!ft_load_texture((*cone)->texture.id, &(*cone)->texture, data)) // loading texture if exists
			ft_error_management(data, 11, (void **)cone, fd);
		if ((*cone)->texture.stretch_x == 0)
			(*cone)->texture.stretch_x = 1;
		if ((*cone)->texture.stretch_y == 0)
			(*cone)->texture.stretch_y = 1;
		if ((*cone)->texture.stretch_z == 0)
			(*cone)->texture.stretch_z = 1;
	}
...
static int ft_cone_values(int *i, t_cone *p, char *s)
{
	if (!(*i & 1) && ft_expect_vector(s, "\tcenter", &(p->center)))
		...
		*i += 16;
	else if (!(*i & 96) && ft_stock_cone_config_next(s, p, i))
		*i += 0;
